...
VAULT_TOKEN=$(curl -sb -X POST -d '{"display_name":"java-app-cerberus-management-service-token"}' -H "X-Vault-Token: ${ROOT_TOKEN}"  http://localhost:8200/v1/auth/token/create | jq -r .auth.client_token)
...
then
    ...
    VAULT_TOKEN=${ROOT_TOKEN} vault write secret/shared/onelogin/config onelogin.api.key=${ONE_LOGIN_API_KEY}
fi
